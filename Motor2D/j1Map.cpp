...
void j1Map::Path(int x, int y)
{
	...
	if (visited.find(goal) != -1) {
		...
		iPoint came_from = breadcrumbs.At(visited.find(goal))->data;
		path.PushBack(came_from);
		while (came_from != visited.start->data) {
			came_from = breadcrumbs.At(visited.find(came_from))->data;
			path.PushBack(came_from);
		}
	}
}
